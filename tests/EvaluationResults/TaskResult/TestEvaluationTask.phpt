...
class TestEvaluationTask extends Tester\TestCase
{
    ...
    public function testMissingRequiredParams()
    {
        ...
        Assert::noError(
            function () {
                new EvaluationTaskResult(['task-id' => 'ABC', 'status' => 'XYZ', 'output' => null]);
            }
        );
        Assert::noError(
            function () {
                new EvaluationTaskResult(['task-id' => 'ABC', 'status' => 'XYZ', 'output' => ""]);
            }
        );
        Assert::noError(
            function () {
                new EvaluationTaskResult(['task-id' => 'ABC', 'status' => 'XYZ', 'output' => "123"]);
            }
        );
    }
    ...
    public function testScoreCalculation()
    {
        ...
        foreach ($judgeToScore as $judgeOutput => $score) {
            $result = new EvaluationTaskResult(
                ['task-id' => 'ABC', 'status' => 'XYZ', 'output' => ["stdout" => $judgeOutput]]
            );
