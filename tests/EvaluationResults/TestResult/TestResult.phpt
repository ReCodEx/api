...
class TestTestResult extends Tester\TestCase
{
    ...
    public function testCalculateStatus()
    {
        ...
        Assert::equal(
            TR::STATUS_SKIPPED,
            TR::calculateStatus(TR::STATUS_SKIPPED, TR::STATUS_OK)
        ); // this should never happen in real world
        Assert::equal(
            TR::STATUS_FAILED,
            TR::calculateStatus(TR::STATUS_FAILED, TR::STATUS_OK)
        ); // this should never happen in real world
        Assert::equal(
            TR::STATUS_SKIPPED,
            TR::calculateStatus(TR::STATUS_SKIPPED, TR::STATUS_FAILED)
        ); // this should never happen in real world
    ...
    public function testTwoExecutionTasks_TimeLimitsExceeded()
    {
        ...
        $res = new TR(
            $cfg, [new ExecutionTaskResult(self::$execRes), new ExecutionTaskResult(self::$execRes2)],
