...
class TestLoader extends Tester\TestCase
{
	...
	private $jobConfigFilePath;

	public function setUp() {
		echo "setup";
		$filePath = tempnam(sys_get_temp_dir(), 'test-job-config-loader');
		file_put_contents($filePath, self::$jobConfig);
		$this->jobConfigFilePath = $filePath;
	}

	public function tearDown() {
		echo "teardown";
		unlink($this->jobConfigFilePath);
		Assert::false(realpath($this->jobConfigFilePath));
	}
