...
class LdapConnectTest extends Tester\TestCase
{
    ...
    public function testErrorCodeExtraction()
    {
        Assert::equal(
            -1,
            LdapUserUtils::getErrorCode(
                "Could not bind privileged user: Ldap Error Code=-1 - Invalid credentials"
            )
        );
        Assert::equal(
            LdapUserUtils::ERROR_WRONG_CREDENTIALS,
            LdapUserUtils::getErrorCode(
                "Could not bind privileged user: Ldap Error Code=49 - Invalid credentials"
            )
        );
        Assert::equal(
            LdapUserUtils::ERROR_TOO_MANY_UNSUCCESSFUL_TRIES,
            LdapUserUtils::getErrorCode(
                "Could not bind privileged user: Ldap Error Code=19 - Constraint violation"
            )
        );
        Assert::equal(
            LdapUserUtils::ERROR_INAPPROPRIATE_AUTHENTICATION,
            LdapUserUtils::getErrorCode(
                "Could not bind privileged user: Ldap Error Code=48 - Inappropriate authentication"
            )
        );
        Assert::equal(
            LdapUserUtils::ERROR_NO_SUCH_OBJECT,
            LdapUserUtils::getErrorCode("Could not bind user: Ldap Error Code=32 - No such object")
        );
    }
    ...
    public function testWrongCredentials()
    {
        $ldapManager = new LdapUserUtils(self::$config);
        Assert::exception(
            function () use ($ldapManager) {
                $ldapManager->getUser("12345678", "password");
            },
            'App\Exceptions\WrongCredentialsException'
        );
    }
