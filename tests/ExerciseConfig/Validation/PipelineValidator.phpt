...
class TestPipelineValidator extends Tester\TestCase
{
  ...
  public function testCorrect() {
    ...
    $dataInBoxMeta->addInputPort(new Port(PortMeta::create(FetchFileBox::$REMOTE_PORT_KEY, "remote-file", "remote-input")));
    ...
    Assert::noError(function () use ($pipelineEntity, $pipeline) {
  ...
  public function testUnusedVariable() {
    ...
    Assert::exception(function () use ($pipelineEntity, $pipeline) {
      $this->validator->validate($pipelineEntity, $pipeline);
  ...
  public function testCannotFindPipelineFile() {
    ...
    $compileBoxMeta = new BoxMeta();
    $compileBoxMeta->setName("compile");
    $compileBoxMeta->addInputPort(new Port(PortMeta::create(GccCompilationBox::$SOURCE_FILES_PORT_KEY, "file", "input")));
    $pipeline->set(new GccCompilationBox($compileBoxMeta));

    Assert::exception(function () use ($pipelineEntity, $pipeline) {
      $this->validator->validate($pipelineEntity, $pipeline);
    }, ExerciseConfigException::class, "#not found in pipeline files#i");
  }
  ...
  private function createPipelineEntity(): PipelineEntity {
    ...
    SupplementaryExerciseFile::fromUploadedFileAndPipeline($uploadedFile, $pipeline, "input.hash", "fileserver.path");
