...
class TestFetchFilesBox extends Tester\TestCase
{
    ...
    {
        $inputPort = (new Port(
            (new PortMeta())->setName(FetchFilesBox::$REMOTE_PORT_KEY)->setType(VariableTypes::$REMOTE_FILE_ARRAY_TYPE)
        ));
        $inputPort->setVariableValue(new Variable(VariableTypes::$REMOTE_FILE_ARRAY_TYPE, "", ["1.in", "2.in"]));

        $outputPort = (new Port(
            (new PortMeta())->setName(FetchFilesBox::$INPUT_PORT_KEY)->setType(VariableTypes::$FILE_ARRAY_TYPE)
        ));
    ...
    public function testSameFileProvidedByUser()
    {
        Assert::exception(
            ...
            "Exercise submission error - File '2.in' is already defined by author of the exercise"
        );
    }
