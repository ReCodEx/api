...
class DirectoriesResolver
{
    ...
    private function processNode(Node $node, string $directoryName, array &$directoriesNodes)
    {
        ...
        foreach ($node->getBox()->getInputPorts() as $inputPort) {
            ...
            if (
                $variableValue && $variableValue->isFile() &&
    ...
    private function createCopyNode(Variable $variable, string $newDirectory): Node
    {
        ...
        if ($variable->isArray()) {
            ...
            $box = (new CopyFilesToDirBox($boxMeta))->setInputPort($inputPort)->setOutputPort(
                $outputPort
            )->setDirectory($newDirectory);
        } else {
            $boxMeta = (new BoxMeta())->setName(CopyFileToDirBox::$COPY_TYPE);
            $box = (new CopyFileToDirBox($boxMeta))->setInputPort($inputPort)->setOutputPort($outputPort)->setDirectory(
                $newDirectory
            );
        }
     ...
     * @throws ExerciseConfigException
     */
    private function addDirectories(
        RootedTree $tree,
        array $directoriesNodes,
        ...
        while (!empty($stack)) {
            ...
            if ($box) {
                foreach ($box->getInputPorts() as $port) {
                    if (
                        !$port->getVariableValue() || !$port->getVariableValue()->isFile() ||
    ...
    public function resolve(RootedTree $tree, CompilationContext $context, CompilationParams $params): RootedTree
    {
        ...
        $stack = array_map(
            ...
            array_reverse($tree->getRootNodes())
        );
        while (!empty($stack)) {
