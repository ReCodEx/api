...
class CASLoginService implements IExternalLoginService
{
    ...
    private function getUserData($ticket, $data, bool $onlyAuthenticate = false): UserData
    {
        try {
            ...
            $this->logger->log(
                "Ticket validation did not return successful response with attributes:\n" . var_export($data, true),
                ILogger::ERROR
            );
            throw new WrongCredentialsException(
        ...
        try {
            ...
            $this->logger->log(
                "The user attributes received from the CAS are incomplete (email missing):\n" . var_export($data, true),
                ILogger::ERROR
            );
            throw new CASMissingInfoException(
        ...
        try {
            ...
            $this->logger->log(
                "The user attributes received from the CAS are missing 'affiliation' for user identification '$ukco'",
                ILogger::WARNING
            );
            $affiliation = [];
        }
        ...
        if (!$role) {
            ...
            $this->logger->log(
                "Given 'affiliation' attributes ($aff) for user '$ukco' does not correspond to any role.",
                ILogger::ERROR
            );
            throw new CASMissingInfoException(
