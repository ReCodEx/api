...
class FileServerProxy
{
    ...
    private function getResultYmlContent(string $zipPath): string
    {
        ...
        if ($yml === false) {
            throw new SubmissionEvaluationFailedException(
                "Results YAML file is missing in the archive received from remote FS."
            );
        }
    ...
    private function decodeJsonResponse(ResponseInterface $response)
    {
        ...
        try {
            ...
            throw new SubmissionFailedException(
                "Fileserver did not respond with a valid JSON response - {$e->getMessage()}"
            );
        }
    }
    ...
    {
        $filesToSubmit = array_map(
            function ($file) {
                if ($file->name === self::JOB_CONFIG_FILENAME) {
                    throw new SubmissionFailedException(
                        "User is not allowed to upload a file with the name of " . self::JOB_CONFIG_FILENAME
                    );
                }
