...
class AnnotationsParser
{
    ...
    /**
     * Returns annotations.
     * @param $r ReflectionClass|ReflectionMethod|ReflectionProperty|ReflectionFunction
     * @return array
     */
    public static function getAll(Reflector $r)
    {
        if ($r instanceof ReflectionClass) {
            ...
            throw new RuntimeException("Unknown Reflector subclass " . get_class($r));
        }

        if (self::$useReflection === null) { // detects whether is reflection available
            // OBSOLETE check, PHP 7+ should be fine with reflection
            self::$useReflection = (bool)(new ReflectionClass(__CLASS__))->getDocComment();
        }
    ...
    public static function expandClassName(string $name, ReflectionClass $reflector): string
    {
        ...
        $parsed = self::getCache()->load($filename, function (&$dp) use ($filename) {
            if (self::$autoRefresh) {
        ...
        foreach ($matches as $match) {
            ...
            if (substr($value, 0, 1) === '(') {
                ...
                while (
                    $m = Strings::match(
                        $value,
                        '#\s*,\s*(?>(' . self::RE_IDENTIFIER . ')\s*=\s*)?(' . self::RE_STRING
                            . '|[^\'"),\s][^\'"),]*)#A'
                    )
                ) {
     ...
     * @return void
     */
    public static function setCacheStorage(Nette\Caching\Storage $storage)
    {
