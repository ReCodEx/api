...
class Version20171108205919 extends AbstractMigration
{
  ...
  public function up(Schema $schema): void
  {
    ...
    $this->addSql('ALTER TABLE solution ADD created_at DATETIME NOT NULL COMMENT \'(DC2Type:datetime)\', CHANGE user_id author_id CHAR(36) DEFAULT NULL COMMENT \'(DC2Type:guid)\'');
  ...
  public function postUp(Schema $schema): void {
    ...
    $result = $this->connection->executeQuery("SELECT submitted_at, solution_id FROM submission WHERE original_submission_id IS NULL");
    foreach ($result as $row) {
      $this->connection->executeQuery("UPDATE solution SET created_at = '{$row['submitted_at']}' WHERE id = '{$row['solution_id']}'");
    }
    ...
    foreach ($result as $row) {
      $this->connection->executeQuery("UPDATE solution SET created_at = '{$row['uploaded_at']}' WHERE id = '{$row['solution_id']}'");
    }
