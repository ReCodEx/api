...
class Version20180316141301 extends AbstractMigration
{
  ...
  private function updateHolyMadafokingPortArray(&$box, &$portArray) {
    foreach ($portArray as $portName => &$port) {
      if ($port["value"] == self::$SOURCE_FILE_PORT) {
        ...
        } else if ($box["type"] == self::$FILE_OUT_TYPE) {
  ...
  private function updatePipeline(string $pipelineName, string $entryBox, bool $hasEntryPoint) {
    $pipeline = $this->connection->executeQuery("SELECT * FROM pipeline WHERE name = :name", ["name" => $pipelineName])->fetch();
    $pipelineConfig = $this->connection->executeQuery("SELECT * FROM pipeline_config WHERE id = :id", ["id" => $pipeline["pipeline_config_id"]])->fetch();
  ...
  private function updateEnvironment(string $environmentId, string $ext) {
    ...
    $configsResult = $this->connection->executeQuery("SELECT * FROM exercise_environment_config WHERE runtime_environment_id = :id",
      ["id" => $environmentId]);
  ...
  private function updateExerciseConfig(array $environments) {
    $passthroughFilePipeline = $this->connection->executeQuery("SELECT * FROM pipeline WHERE name = :name",
      ["name" => "Compilation source file pass-through"])->fetch()["id"];
  ...
  private function addSolutionParams(string $environmentId) {
    $solutionsResult = $this->connection->executeQuery("SELECT * FROM solution WHERE runtime_environment_id = :id",
      ["id" => $environmentId]);
    foreach ($solutionsResult as $solution) {
      $filesResult = $configsResult = $this->connection->executeQuery("SELECT * FROM uploaded_file WHERE solution_id = :id",
        ["id" => $solution["id"]]);
      ...
      $this->connection->executeQuery("UPDATE solution SET solution_params = :params WHERE id = :id",
