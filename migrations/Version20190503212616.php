...
final class Version20190503212616 extends AbstractMigration
{
    ...
    {
        $this->failures = $this->connection->executeQuery("SELECT id, reference_solution_submission_id, assignment_solution_submission_id FROM submission_failure")->fetchAll();
    }
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('ALTER TABLE assignment_solution_submission ADD failure_id CHAR(36) DEFAULT NULL COMMENT \'(DC2Type:guid)\'');
        $this->addSql('ALTER TABLE assignment_solution_submission ADD CONSTRAINT FK_114838A3BADC2069 FOREIGN KEY (failure_id) REFERENCES submission_failure (id)');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_114838A3BADC2069 ON assignment_solution_submission (failure_id)');
        $this->addSql('ALTER TABLE reference_solution_submission ADD failure_id CHAR(36) DEFAULT NULL COMMENT \'(DC2Type:guid)\'');
        $this->addSql('ALTER TABLE reference_solution_submission ADD CONSTRAINT FK_AA9C8B99BADC2069 FOREIGN KEY (failure_id) REFERENCES submission_failure (id)');
        ...
        $this->addSql('ALTER TABLE submission_failure DROP reference_solution_submission_id, DROP assignment_solution_submission_id');
    }
    ...
    public function postUp(Schema $schema): void
    {
        foreach ($this->failures as $failure) {
            if ($failure['assignment_solution_submission_id']) {
                $this->connection->executeQuery('UPDATE assignment_solution_submission SET failure_id = :failureId WHERE id = :submissionId',
    ...
    {
        $this->assignmentSubmissions = $this->connection->executeQuery("SELECT id, failure_id FROM assignment_solution_submission WHERE failure_id IS NOT NULL")->fetchAll();
        $this->referenceSubmissions = $this->connection->executeQuery("SELECT id, failure_id FROM reference_solution_submission WHERE failure_id IS NOT NULL")->fetchAll();
    }
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('ALTER TABLE submission_failure ADD reference_solution_submission_id CHAR(36) DEFAULT NULL COLLATE utf8mb4_unicode_ci COMMENT \'(DC2Type:guid)\', ADD assignment_solution_submission_id CHAR(36) DEFAULT NULL COLLATE utf8mb4_unicode_ci COMMENT \'(DC2Type:guid)\'');
    ...
    public function postDown(Schema $schema): void
    {
        foreach ($this->assignmentSubmissions as $submission) {
            $this->connection->executeQuery('UPDATE submission_failure SET assignment_solution_submission_id = :submissionId WHERE id = :failureId',
                [ 'submissionId' => $submission['id'], 'failureId' => $failsubmissionure['failure_id'] ]);
        }
        foreach ($this->referenceSubmissions as $submission) {
            $this->connection->executeQuery('UPDATE submission_failure SET reference_solution_submission_id = :submissionId WHERE id = :failureId',
                [ 'submissionId' => $submission['id'], 'failureId' => $failsubmissionure['failure_id'] ]);
        }
    }
}
