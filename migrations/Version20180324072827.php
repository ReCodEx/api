...
class Version20180324072827 extends AbstractMigration
{
  ...
  private function updatePassthroughEnvironments() {
    ...
    $phpid = "php-linux";
    $nodeid = "node-linux";
    $this->connection->executeQuery("DELETE FROM pipeline_runtime_environment WHERE pipeline_id = :pid AND runtime_environment_id = :phpid",
  ...
  private function getEntryPointsRelevantPipelinesIds(): array {
    ...
    foreach ($pipelinesNames as $name) {
      $pipelines[] = $this->connection->executeQuery("SELECT * FROM pipeline WHERE name = :name", ["name" => $name])->fetch()["id"];
    }
